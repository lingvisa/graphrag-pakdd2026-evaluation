...
## Semantic Alignment Score Calculation

The aggregate **Semantic Alignment Score** across all evaluation instances is computed as:
```
Semantic Alignment Score = (0.5 × P₀.₅ + 1.0 × P₁.₀) × 100%
```

Where:
- **P₀.₅** = Proportion of responses receiving score 0.5 (partial coverage)
- **P₁.₀** = Proportion of responses receiving score 1.0 (full coverage)
- **P₀** = Proportion receiving score 0 (no coverage) - not included in weighted calculation

This weighted average emphasizes complete answers while giving partial credit to responses that capture some essential information. The metric is particularly suitable for technical domains where partial correctness is meaningful but incomplete answers may still fail to solve the user's problem.

### Interpretation:
- **High score (>70%):** Most responses fully or substantially address questions
- **Medium score (50-70%):** Mix of complete and partial responses
- **Low score (<50%):** Frequent gaps in coverage or incorrect information
...
- **Domain:** Technical code migration and system conversion
- **Question Types:** 
  - Procedural queries (how-to questions)
  - Troubleshooting (error resolution)
  - Compatibility checks (deprecated component handling)
  - Best practices (migration strategies)
- **Complexity:** Questions require technical understanding of legacy system architecture, API changes, and migration patterns
...
### Judge Model:
- LLM-based evaluation using structured prompting
- Discrete scoring (0, 0.5, 1) for clear differentiation
- JSON output format for programmatic processing

### Quality Control:
- Ground truth answers created by domain experts
- Consistent scoring criteria across all evaluations
- Focus on semantic content rather than surface form

### Comparison Baseline:
This evaluation enables direct comparison of retrieval methods:
1. **Dense Vector Retrieval** - Semantic similarity-based baseline
...
Evaluation results demonstrate the effectiveness of graph-structured retrieval:

| Metric | Dense Vector | GraphRAG (GPT-4o) | GraphRAG (Dependency) |
|--------|-------------|-------------------|----------------------|
| **No Coverage (0)** | 40.29% | 27.34% | 27.34% |
| **Partial Coverage (0.5)** | 15.85% | 13.67% | 21.58% |
| **Full Coverage (1.0)** | 42.88% | 58.99% | 51.08% |
| **Weighted Average** | 50.80% | 65.83% | 61.87% |

### Improvement Analysis:

**No Coverage Reduction:**
```
Absolute reduction: 40.29% → 27.34% = 12.95 percentage points
Relative reduction: (40.29% - 27.34%) / 40.29% = 32.1%
```

**Full Coverage Improvement:**
```
GPT-4o: (58.99% - 42.88%) / 42.88% = 37.6% relative increase
Dependency: (51.08% - 42.88%) / 42.88% = 19.1% relative increase
```

**Weighted Average Improvement:**
```
GPT-4o: 65.83% - 50.80% = 15.03 percentage points
Dependency: 61.87% - 50.80% = 11.07 percentage points
```

### Key Findings:
- GraphRAG reduces no-coverage failures by **32% (relative reduction)**, from 40.29% to 27.34%
- Full-coverage rate improves by **19-37% (relative improvement)**, from 42.88% to 51.08-58.99%
- Overall semantic alignment increases by **11-15 percentage points (absolute increase)**, from 50.80% to 61.87-65.83%
